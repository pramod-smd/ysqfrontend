...
export const editCustomer =
    (customerId, customer, navigate) => async (dispatch) => {
        dispatch(setSavingButton(true));
        const { name, dob, email, phone, country, city, address,postal_code,latitude,longitude,image,user_id,
            chanel_id,
            credit_limit,area_id, sub_area_id, status } = customer;
        const data = {
            name,
...
export const fetchAllCustomer = (filterDate) => async (dispatch) => {
    apiConfig
        .get(`customers?page[size]=0&filterDate=${filterDate}`,)
        .then((response) => {
            dispatch({
...
export const fetchAllCustomerList = (filterDate) => async (dispatch) => {
    apiConfig
        .get(`customer-list`)
        .then((response) => {
            dispatch({
                type: customerActionType.FETCH_ALL_CUSTOMER_LIST,
                payload: response.data.data,
            });
        })
        .catch(({ response }) => {
