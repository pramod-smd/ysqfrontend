...
export const fetchCustomersReport =
    ...
    async (dispatch) => {
        ...
        apiConfig
            ...
            .then((response) => {
                dispatch({
                    type: customerActionType.FETCH_CUSTOMERS_REPORT,
...
export const fetchCustomerSalePayment =
    ...
    async (dispatch) => {
        ...
        await apiConfig
            ...
            .then((response) => {
                ...
                dispatch({
                    type: customerActionType.FETCH_CUSTOMERS_PAYMENT_REPORT,
                    payload: response.data.data.data,
                });
    ...
    export const customerGiftReportExcel = (customer_id,dates,warehouseValue, setIsWarehouseValue, filter = {}, isLoading = true) => async (dispatch) => {
        if (isLoading) {
            dispatch(setLoading(true))
        }
        await apiConfig.get(`total-gift-report-excel?start_date=${dates?.start_date ? dates?.start_date : null }&end_date=${dates?.end_date ? dates?.end_date : null}&warehouse_id=${warehouseValue?.value ? warehouseValue?.value : null}&customer_id=${customer_id ? customer_id : null }`)     
            .then((response) => {
...
export const customerPdfAction =
...
export const customerSaleReportExcel = (id) => async (dispatch) => {
    apiConfig
        .get(apiBaseURL.CUSTOMER_SALES_REPORT_EXCEL + "/" + id)
        .then((response) => {
            window.open(response.data.data.customer_sale_excel_url, "_blank");
        })
        .catch(({ response }) => {
            dispatch(
                addToast({ text: response.data.message, type: toastType.ERROR })
...
export const customerSaleReturnReportExcel = (id) => async (dispatch) => {
    apiConfig
        .get(apiBaseURL.CUSTOMER_SALES_RETURNS_REPORT_EXCEL + "/" + id)
        .then((response) => {
            window.open(response.data.data.customer_sale_return_excel_url, "_blank");
        })
        .catch(({ response }) => {
            dispatch(
                addToast({ text: response.data.message, type: toastType.ERROR })
