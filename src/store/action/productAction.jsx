...
export const fetchAllMainProductsList = (filter = {}, isLoading = true) => async (dispatch) => {
    if (isLoading) {
        dispatch(setLoading(true));
    }
    let url = apiBaseURL.MAIN_PRODUCTS_LIST;
    if (
        !_.isEmpty(filter) &&
        (filter.page ||
            filter.pageSize ||
    ...
    apiConfig
        ...
        .then((response) => {
            dispatch({
                type: productActionType.MAIN_PRODUCTS_LIST,
                payload: response.data.data,
            });
