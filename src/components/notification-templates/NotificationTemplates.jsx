...
const NotificationTemplates = ( props ) => {
    const { totalRecord, isLoading, smsTemplates, fetchSmsTemplates, activeInactiveSms } = props;
    ...
    const goToEditProduct = ( item ) => {
        const id = item.id
        navigate( `/app/notification-templates/${id}` )
    };
    ...
    return (
        <MasterLayout>
            <TopProgressBar />
            <TabTitle title="Notification templates" />
            <ReactDataTable 
            columns={columns} 
            items={itemsValue} 
            onChange={onChange} 
            ...
            ButtonValue="Create Notification"
...
export default connect( mapStateToProps, { fetchSmsTemplates, activeInactiveSms } )( NotificationTemplates );
