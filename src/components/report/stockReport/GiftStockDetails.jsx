...
const GiftStockDetails = (props) => {
    ...
    return (
        <MasterLayout>
            <TopProgressBar />
            <HeaderTitle title="" to='/app/report/report-inventory-gifts'/>
                        ...
                        {stockWarehouse && stockWarehouse.map((warehouse, index) => {
                            return (
                                <tr key={index}>
                                    ...
                                    <td>{warehouse.quantity} {stockWarehouse[0] && stockWarehouse[0].product.product_unit === "1" && "Pc" || stockWarehouse[0].product.product_unit === "2" && "M" || stockWarehouse[0].product.product_unit === "3" && "Kg"}</td>
                                </tr>
                            )
                        })}
                ...
                <Tabs defaultActiveKey='sales' id='uncontrolled-tab-example' onSelect={(k) => setKey(k)}
                    ...
                    <Tab eventKey='sales' title={getFormattedMessage('sales.title')}
                        ...
                        <div className='w-100 mx-auto'>
                            {key === 'sales' && <SaleTabs allConfigData={allConfigData} id={id} warehouseValue={stockWarehouse && stockWarehouse[0]?.warehouse_id}/>}
                        </div>
                    </Tab>
                    <Tab eventKey='sales-return' title={getFormattedMessage('sales-return.title')}
                         tabClassName='position-relative mb-3 me-7'>
                        <div className='w-100 mx-auto'>
                            {key === 'sales-return' && <SaleReturnTabs allConfigData={allConfigData} id={id} warehouseValue={stockWarehouse && stockWarehouse[0]?.warehouse_id}/>}
                        </div>
                    </Tab>
...
export default connect(mapStateToProps, {stockDetailsWarehouseAction})(GiftStockDetails);
