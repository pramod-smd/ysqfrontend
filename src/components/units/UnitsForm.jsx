...
const UnitsForm = (props) => {
    ...
    const newUnit =
        singleUnit &&
        base.filter((da) => singleUnit.base_unit === da.attributes.name);
    ...
    useEffect(() => {
        if (singleUnit) {
            const data = base.filter(
                (da) => Number(singleUnit.base_unit) === da.id
            );
            data.length &&
                setUnitValue({
                    name: singleUnit ? singleUnit.name : "",
                    short_name: singleUnit ? singleUnit.short_name : "",
                    base_unit: {
                        label: data[0]?.attributes?.name,
                        value: singleUnit?.base_unit,
                    },
                });
        }
    }, [singleUnit]);
    ...
    const disabled =
        singleUnit &&
        singleUnit.name === unitValue.name.trim() &&
        singleUnit?.short_name === unitValue?.short_name.trim() &&
        unitValue?.base_unit[0] &&
        unitValue?.base_unit[0]?.label === singleUnit?.base_unit;
    const [selectedBaseUnit] = useState(
        newUnit
            ? [{ label: newUnit[0]?.attributes?.name, value: newUnit[0]?.id }]
            : null
    );
    ...
    const handleValidation = () => {
        ...
        if (!unitValue["name"].trim()) {
            ...
            errorss["short_name"] = getFormattedMessage(
                "unit.modal.input.short-name.validate.label"
            );
        } else if (
            unitValue["short_name"] &&
            unitValue["short_name"].length > 50
        ) {
            errorss["short_name"] = getFormattedMessage(
                "unit.modal.input.short-name.valid.validate.label"
            );
        } else if (!unitValue["base_unit"]) {
            errorss["base_unit"] = getFormattedMessage(
                "unit.modal.input.base-unit.validate.label"
            );
        } else {
            isValid = true;
        }
                ...
                <Modal.Body>
                    <div className="row">
                        ...
                        <div className="col-md-12 mb-3">
                            <label className="form-label">
                                {getFormattedMessage(
                                    "unit.modal.input.short-name.label"
                                )}
                                :{" "}
                            </label>
                            <span className="required" />
                            <input
                                ...
                                placeholder={placeholderText(
                                    "unit.modal.input.short-name.placeholder.label"
                                )}
                                onChange={(e) => onChangeInput(e)}
                            />
                            <span className="text-danger d-block fw-400 fs-small mt-2">
                        ...
                        <div className="col-md-12 mb-3">
                            <ReactSelect
                                title={getFormattedMessage(
                                    "unit.modal.input.base-unit.label"
                                )}
                                placeholder={placeholderText(
                                    "unit.modal.input.base-unit.placeholder.label"
                                )}
...
export default connect(mapStateToProps, { fetchAllBaseUnits, editUnit })(
    UnitsForm
);
