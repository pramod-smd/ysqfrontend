...
import { CheckOutLists } from "../../store/action/surveyAction";
const CheckIn = (props) => {
    const {
        fetchExpenses,
        expenses,
    ...
    const onChange = (filter) => {
        CheckOutLists(filter, true);
    };

    const onDetailsClick = (item) => {
        const id = item;
        if (id) {
            window.location.href = "#/app/checkout-details/" + item;
        } else {
    ...
    return (
        <MasterLayout>
            <TopProgressBar />
            <TabTitle title={placeholderText("checkout.title")} />
            <ReactDataTable
                columns={columns}
                items={itemsValue}
                onChange={onChange}
...
export default connect(mapStateToProps, { CheckOutLists })(CheckIn);
